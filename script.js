// ///////////// DISPLAYS IN THE CONSOLE AND DATE TIME ON THE WEBPAGE! ///////////////
...
/////////////////// FROM SCRATCH - SHOWCASING THE RESULT ON THE PAGE INSTEAD OF IN THE CONSOLE ///////////////////////
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Function to handle user input and display results in the HTML
    function handleUserInput() {
        submit.addEventListener('click', function () {
            // Parse the input value from the textbox
            let inputValue = parseInt(myvalue.value);
            ...
            // Check if the input is a valid number
            if (!isNaN(inputValue)) {
                // Display the result in the HTML instead of logging it to the console
                dateresultElement.innerHTML = `<h1>${inputValue + 2}</h1>`; // Show calculated value on HTML page
            } else {
                // Display an error message in the HTML
                dateresultElement.innerHTML = `<h1>Please enter a valid number.</h1>`; // Show error message on HTML page
            }
        });
    }
